...
class Turno:
    def __init__(self, fecha_hora, servicio, cliente_nombre ):
    ...
    def __str__(self):
        return f"{self.cliente_nombre} | {self.servicio.nombre} | {self.fecha_hora.strftime('%d/%m/%Y %H:%M')} | {self.estado}"
...
class AppTurnos: # Clase gestora de la ventana principal y sus interacciones.
    ...
    def abrir_reserva(self): 
        ...
        def confirmar_reserva():
            ...
            for t in usuario_actual.turnos:                         # Validacion de duplicado de turnos
                if t.fecha_hora == fecha_hora and t.servicio == servicio:
                 messagebox.showerror("Turno existente", "Ya tenés un turno reservado en esa fecha y hora.")
                return
            ...
            turno = Turno(fecha_hora, servicio, nombre_apellido.strip()) # Creación y guardado del turno.
